...
# read in S2 file
full_df = read.table("S2_extracted_intensities (4).txt", header = T, sep = "\t", check.names = F) 
...
# get_PTM_sites <- function(x){ # not used
#   temp = str_extract_all(x, "(?<=[[:alpha:]])[[:digit:]]+(?=\\: [[:alpha:]])")
#   temp = sort.int(na.omit(as.numeric(unique(unlist(temp)))))
#   return(temp)
# }
...
# get_PTM_types <- function(x){ # not used
#   temp = str_extract_all(x, "[[:alpha:]]+[[:digit:]]+\\: [[:alpha:]]+")
#   temp = unique(unlist(temp))
#   temp = gsub("^.*\\: ", "", temp)
#   return(temp)
...
write.csv(PTM7, file = "TNBC Histone.csv", row.names = F, quote = F)
...
#   write_tsv("data/S3_single_spread_output.txt")
# 
# 
